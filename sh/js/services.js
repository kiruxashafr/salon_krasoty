...
function filterServicesWithAvailability(services) {
            ...
            fetch(`/api/service/${service.id}/specialists`)
                ...
                .then(data => {
                    if (data.message === 'success' && data.data && data.data.length > 0) {
                        // Проверяем доступность с учетом временных слотов
                        checkServiceHasValidTimeSlots(service.id, data.data, startDate, endDate)
    ...
    Promise.all(servicePromises)
        .then(results => {
            ...
            console.log('Available services with valid time slots:', availableServices);
...
// Новая функция для проверки доступности услуг
function checkServiceHasValidTimeSlots(serviceId, specialists, startDate, endDate) {
    ...
    const specialistPromises = specialists.map(specialist => {
        return new Promise((resolve) => {
            fetch(`/api/specialist/${specialist.id}/service/${serviceId}/available-dates?start=${startDate}&end=${endDate}`)
                .then(response => {
                    if (!response.ok) {
                        resolve(false);
                        return;
                ...
                .then(data => {
                    ...
                    // Проверяем каждый день на наличие валидных слотов
                    const datePromises = data.availableDates.map(date => {
                        return checkDateHasValidTimeSlotsService(specialist.id, serviceId, date);
                    });
        ...
        return fetch(`/api/specialist/${specialist.id}/service/${serviceId}/available-dates?start=${startDate}&end=${endDate}`)
            .then(response => {
                ...
                return response.json().then(data => {
                    ...
                    const datePromises = data.availableDates.map(date => {
                        return checkDateHasValidTimeSlotsService(serviceId, specialist.id, date);
                    });
...
function generateDateGridService(availableDates) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (!isPast && availableDates.includes(formattedDate)) {
            datePromises.push(
                checkDateHasValidTimeSlotsService(window.serviceCurrentSpecialistId, window.serviceCurrentServiceId, formattedDate)
